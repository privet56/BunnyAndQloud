...
//! [top]
    ...
    function addMarkerAtPos(longi, lati, userdata)
    {
        var count = map.markers.length
        markerCounter++
        var marker = Qt.createQmlObject ('Marker {}', map, {userdata:userdata})
        map.addMapItem(marker)
        marker.z = map.z+1;

        marker.coordinate = map.toCoordinate(Qt.point(longi, lati));
        /*marker.coordinate = {
            latitude: longi,
            longitude: lati
        };*/
    ...
    function addImgGeoItemAtPos(item, pos, imgSrc, userdata, onDblClick, longi, lati)
    {
        var count = map.mapItems.length
        var co = Qt.createComponent(item+'.qml')
        if (co.status == Component.Ready)
        {
            var o = co.createObject(map, {"imgSrc":imgSrc, userdata: userdata})
            o.onItemDblClicked.connect(onDblClick)
            o.imgSrc = imgSrc
            o.setGeometry(map.markers, pos ? pos : map.toCoordinate(Qt.point(longi, lati)))
            o.userdata = userdata;
            map.addMapItem(o)
            //update list of items
            var myArray = new Array()
...
//! [coord]
    ...
    center
	{
        //latitude: 59.9485         // The Qt Company in Oslo
        //longitude: 10.7686
        latitude: 48.0978           // Munich: 48.0978,11.5231
        longitude: 11.5231
    }
//! [coord]
...
*/
    ...
    Item {
        ...
        Image {
            id: scaleImageLeft
            source: "../../res/scale_end.png"
            anchors.bottom: parent.bottom
            anchors.right: scaleImage.left
        }
        Image {
            id: scaleImage
            source: "../../res/scale.png"
            anchors.bottom: parent.bottom
